...
- **Routes:** Intro (`/`) plays the VC mark then slides to the unified landing (`/menu`). The door state shows **Enter the Club** + Settings; after entry it redirects to `/bar`, keeps the shelf/bar scene mounted, and presents the restaurant-style club menu with game landing cards. **Oubliette No. 9** (`/minigames/oubliette-no9`) and **7 Year Itch** (`/minigames/seven-year-itch`) start from those landing cards; `/bar` deep links reuse the same unified host and still show optional settlement flash state; **`/__playground`** remains linked from the club menu.
