...
import type { ClubTableReturnDetail, MastersonShellBinding } from "@/game/sessionSettlement";
...
export function MastertonPage() {
  ...
  useEffect(() => {
    document.title = "Masterton 1881 — The Villains Club";
  }, []);

  useEffect(() => {
    if (isReturningToClubRef.current) return;
    if (!activeSession || activeSession.gameId !== "masterson_1881") {
  ...
  const handleReturnToClub = useCallback(
    (detail: ClubTableReturnDetail) => {
      ...
      const gameId = snap?.gameId ?? "masterson_1881";
  ...
  const shellProps = useMemo((): MastersonShellBinding | null => {
    if (!activeSession || activeSession.gameId !== "masterson_1881") return null;
    return {
  ...
  return (
    <Box
      ...
      style={{
        ...
        background: "radial-gradient(ellipse at 50% 0%, rgba(139, 92, 26, 0.12), transparent 55%), #0f0d0b",
          ...
          fallback={
            ...
            >
              ...
              <Text size="sm" c={clubTokens.text.muted} ta="center">
                Entering Masterton 1881…
              </Text>
            </Box>
          }
        >
          <MastertonRoot {...shellProps} />
        </Suspense>
