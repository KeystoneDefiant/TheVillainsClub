...
- **Minigame Rules configurations & overrides:** Extracted all static configurations out of minigame rules files into dedicated config modules (`oublietteNo9Config.ts`, `sevenYearItchConfig.ts`, `fatesealConfig.ts`, `mastersonConfig.ts`, `ligneeRoyaleConfig.ts`) to make game modes and parameters easier to read and edit. Merged dynamically using imports/exports.
- **Oubliette port:** First-party copy under **`src/minigames/oubliette-no9/`**; table rules config at **`src/config/minigames/oublietteNo9GameRules.ts`** (`resolveOublietteGameMode` wrapper of `oublietteNo9Config.ts`); runtime mode is provided by **`OublietteGameModeProvider`** on **`OublietteNo9Page`** so hooks and shop math track the session profile. Tailwind + theme SCSS loaded from **`OublietteNo9Page`**. Shell-bound sessions snapshot Oubliette state for resume, voluntary cash-out is available at round 31+, and upgraded large hand reveal animation supporting selectable Felt Cascade (Option A) and Arc Carousel (Option C) history stack styles with shared card stage presence and implicit golden credit streams.
...
- **Fateseal Silver:** Occult cascading grid slot under **`src/minigames/fateseal-silver/`**; design spec **`Fateseal_Specs.md`**. Occult symbols, rules, and multipliers structured in **`src/config/minigames/fatesealConfig.ts`**. Altar single omen pick; Crossroads offers additional omens. Bet button controls, scatter append waves, transient void columns, FIFO paid-spin timers, and wild decay. Automatic Cash-out Safeguard triggers when session credits fall below bet limits. Features an occult, spooky visual theme including: deep obsidian felt gradients, edge vignettes that pulse with dark crimson/purple energy depending on game activity (busy spins/cascades), unstable obsidian runic cells that flicker on hover, erupting magical cascade matching animations, a complex rotating multi-ring runic background seal with an alchemical heptagram and sulfur marker, and full visual corruption edge-vein integration when the Forbidden Tome is active.
...
- **Lignée Royale:** Private 7-row playing card slot machine minigame under **`src/minigames/lignee-royale/`**; configuration in `ligneeRoyaleConfig.ts` (normal and high stakes profiles); features dynamic payline overlay SVG vectors, custom Wild Multiplier gradients (up to 5x stacking), Dead Space card exclusions, vertical motion blurs, snappy whole-reel bounce settles, dimmed non-scoring backdrop rows, swapped spin layouts, nested paylines results, settings modal gear integration, and a **Hold-and-Respin bonus game** triggered by 3+ scatters, complete with coin collect multipliers (3x-1000x), dynamic countdown/payout overlays, custom card faces (👑, 🪙), and independent card spinning/bouncing animations.
