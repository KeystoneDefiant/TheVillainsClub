...
- **Game Scale Container:** The polymorphic auto-scaling wrapper `GameScaleContainer` (`src/components/ui/GameScaleContainer.tsx`) scales game screens (`OublietteNo9Page`, `SevenYearItchPage`, `FatesealSilverPage`, `MastertonPage`) to fit window/viewport boundaries using CSS `transform: scale()`. This preserves exact layout proportions and interactive hitboxes on mobile/tablets while enabling large screens to expand the Speakeasy felt and grid without pixel clipping. Supports custom `transformOrigin` and `alignItems` props to allow top-aligned flush scaling.
