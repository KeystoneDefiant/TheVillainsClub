...
- **Minigame Rulesets, Dynamic Buy-ins, & Overrides Display:** All minigames dynamically parse and render their available configurations (`gameModes` overrides) inside the Ruleset dropdown on the main menu, instead of hardcoding static "House Rules". The ruleset selection is rendered using our custom `<ClubSelect fancy variant="light" />` for a premium look. When a non-default mode is selected, a visually premium "Ruleset Modifications" comparison panel instantly unfurls underneath the Select component, highlighting parameter differences (such as custom buy-in wagers, payout limits, starting credits, base bet scales, or player count limits). Starts tables and resolves settlement cap calculations dynamically using the chosen ruleset's specific overrides.
- **Game Tips Modal:** Integrates a game-specific strategy tips overlay modal (`GameTipsModal.tsx`) launchable directly from the game's details card on the Main Menu. Curated tips configuration catalogs (`gameTipsCatalog.ts`) cover critical strategic guidelines for all four minigames. Restructured the tips content to mimic the Sommelier live tutorial layout, with tip narration and dialogue written in double-quotes and italics, spoken by **Claudius L'Ausula** (Club Pit Boss, emoji: 🎲). Integrates smooth fade/slide transitions between tip pages, and automatically animates container height differences (honoring `prefers-reduced-motion`).
