...
- **Routes:** Intro (`/`) → main menu (`/menu`) → club floor (`/bar`, table buy-ins) → **Oubliette No. 9** (`/minigames/oubliette-no9`) or **7 Year Itch** (`/minigames/seven-year-itch` after `startSession`), return to **`/bar`** with optional flash state on settle; **`/__playground`** (UI lab — linked from main menu and bar).
...
- **Audio:** **`src/audio/clubAudioStore.ts`** (persisted) is the single settings source for **music and SFX** (main menu Settings). Oubliette plays **table/UI SFX only** via `useThemeAudio` reading that store; it does not duplicate audio controls or start its own background music. **House band** (main menu / bar music) **continues on `/minigames/*`** via `useShellBandMusic` so shell and minigames share one stream. Tracks from `content/bands.json` resolve under `public/audio/bands/*` via **`bandPublicUrl`**: root-absolute under **http(s)** (GitHub Pages `/repo/menu` → `/repo/audio/...`), path-relative under **`file:`** (Electron `dist/index.html` next to `audio/`). **`/__playground` → House bands** tab can override the evening band index and spot-check each track.
