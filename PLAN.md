...
- **Renderer:** Vite + React 19, **Mantine** UI, **Framer Motion** for intro/menu motion. **Intro (`/`):** animated VC mark from **`public/images/logos/VC Logo - Color.svg`** (paths in `src/components/intro/vcLogoIntroPaths.ts`): red fills zoom out while each grey letter **fills** in with a **bottom→top** linear-gradient reveal (no stroke outline); timing via `introLogoLetterDrawSec`, `introLogoSettleSec`, `introHoldSec`.
