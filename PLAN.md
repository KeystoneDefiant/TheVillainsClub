...
- **Fateseal Silver:** Occult cascading grid slot under **`src/minigames/fateseal-silver/`**; weighted symbol pool, prophecy (single/triple focus), cascades + cluster clears, scatter → Free Ritual, Crossroads shop every 3 spins (`fatesealCrossroadsEveryNSpins` in **`src/config/minigames/fatesealRules.ts`**). Pure engine in **`src/minigames/fateseal-silver/engine/`**. Monte Carlo harness: **`npm run sim:fateseal`** (`scripts/sim-fateseal.ts`).
