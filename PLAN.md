...
- **7 Year Itch:** Crapless craps minigame under **`src/minigames/seven-year-itch/`**; NV paytables, racket lore, and heat bonus config in **`src/config/minigames/sevenYearItchRules.ts`**; agent plan **`7YI_plan.md`**. The UI is compact/no-scroll oriented, prioritizes story modals, heat offers configured favor/devil-deal style choices every four non-7 rolls, and cash-out is allowed only when no point/case is active.
