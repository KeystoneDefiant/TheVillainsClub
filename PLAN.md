...
- **Renderer:** Vite + React 19, **Mantine** UI, **Framer Motion** for intro/menu motion. **Intro (`/`):** animated VC mark from **`public/images/logos/VC Logo - Color.svg`** (paths duplicated in `src/components/intro/vcLogoIntroPaths.ts` for stroke animation): red fills zoom out while grey shapes draw in; timing via `introLogoLetterDrawSec`, `introLogoSettleSec`, `introHoldSec`.
...
- **Audio:** **`src/audio/clubAudioStore.ts`** (persisted) is the single settings source for **music and SFX** (main menu Settings). **Shell house band:** `content/bands.json` + **`public/audio/bands/`** — one band per local bar day (4:00 boundary), shuffled play-through of tracks with random interludes; **`ShellBandMusicHost`** on `/menu` and `/bar` only. Oubliette plays **table/UI SFX only** via `useThemeAudio` reading that store; it does not duplicate audio controls or start its own background music.
