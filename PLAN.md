...
- **7 Year Itch:** Crapless craps minigame under **`src/minigames/seven-year-itch/`**; NV paytables, racket lore, heat bonuses, **`sevenYearItchGameConfig`** (same **default + `gameModes` merge** pattern as Oubliette’s `gameConfig`; resolved table rules in **`sevenYearItchTableConfig`** / **`getSevenYearItchGameMode`**), and **per-roll noir lines** in **`src/config/minigames/sevenYearItchRollStories.ts`**; agent plan **`7YI_plan.md`**. UX: pass-only come-out → full layout with point; **hand recap is merged into the roll story modal** with continue / cash-out; favors as a full view; **Look the other way** keeps the **whole layout**; **Divest** / **Clean Getaway**; optional Field/Horn; cash-out only when no point is active. Mobile view optimized to allow vertical scrolling (unconstraining height and overflow on screens <= 520px), wrapping controls row (dice centered above side-by-side buttons, stretching roll to 100% on come-out layout), and stacking bottom actions to prevent layout overlap or spill-out. **High-Fidelity UX Refinements:** Places classy absolute 22px circular brass-rimmed number badges in the top-left corner of place bet cells, increasing font size to a bold 0.82rem; displays horizontal max-bet wagers progress bars relative to the place bet limit (`Pass Line * 3`); dynamically locks the betting unit (`chip` increment/decrement) equal to the initial Come Out Pass Line seed investment once a point is active; registers contextmenu event preventions on document mount to block native right-click prompts; and suppresses the automatic roll story modal (`loreOpen`) popup to guarantee incredibly fast and fluid consecutive rolls.
- **Fateseal Silver:** Occult cascading grid slot under **`src/minigames/fateseal-silver/`**; design spec **`Fateseal_Specs.md`**. **Shell:** single-symbol seal at the altar; additional omens only via Crossroads random “add symbol” offer (if < 4 symbols). Ritual stakes via **100 / 200 / 500 / 1000** bet buttons (responsive, full width container); legacy **`freeRitualSpinsLeft`** path remains for zero-bet spins but the scatter meter **no longer banks** charges — meter fires **append bonus waves** inside the same `runSpin` (grid grows up to **`bonusGrid.maxGridSize`**, transient bonus dead columns, Sympathetic at configured fire count). Weighted symbol pool, cascades with gravity shifting void symbols to lowest empty spots and protecting scatter symbols from wild overwrites. **`fatesealCrossroadsNewShop`** — wild/dead purchases use **FIFO paid-spin timers** with **column fill masks** (wild left / purchased dead right) and active wild chance decays exponentially per cascade depth step (via `fatesealProgressionRules.purchasedReels.wildChanceDecayPerDepth`) to prevent infinite cascade loops. Crossroads includes **Unsettle the Spirits** (free wild spins at bet 250), **Faustian Bargain** (dead reels for credits, locking bet to 250 and disabling cash-out), and **Vassago's Gambit** (guaranteed scatter bonus next spin, but scatters don't accumulate Crossroads progress; adds an extra normal reel with symbols instead of a void reel, and increases scatter chances by 50% configurable in settings). **`fatesealGameConfig`** (**default + `gameModes` merge**, `resolveFatesealGameMode`) for wager levels and powerups. **Automatic Cash-out Safeguard**: Integrated an automatic cash-out hook that triggers instantly when the player's session wallet falls below the minimum bet size (or below the forced 250 credits Faustian Bargain bet). This overrides all standard cash-out lock rules to guarantee players are never locked or stuck when they are broke. Pure engine in **`src/minigames/fateseal-silver/engine/`**. Shell UI: **altar → ritual (grid + cascades) → ledger**, plus in-flow **`crossroads`** phase (full-width shop, not a modal) when the scatter bank trips. Monte Carlo: **`npm run sim:fateseal`** (`scripts/sim-fateseal.ts`; use **`npx tsx scripts/sim-fateseal.ts`** if `tsx` is not on PATH); use **`FATESEAL_SIM_BASE_ONLY=1`** when tuning the scale toward the staged ~90% payout on paid bets in the stripped base game.
- **Settlement → bar handoff:** Each minigame page (`OublietteNo9Page`, `SevenYearItchPage`, `FatesealSilverPage`, `MastertonPage`) holds an **`isReturningToClubRef`** that gates the "no active session → redirect to /menu" effect; the cash-out handler flips the ref before calling `endSession`, so the immediate `navigate("/bar", { state: BarRouteState })` is not tramped by the redirect-to-menu effect that fires once `activeSession` becomes null. Without this guard, `ClubSettlementDock` (the bar-side quip card) never receives `lastTable` state.
- **Dynamic Game Scaling to Fit Viewport:** Integrates custom `GameScaleContainer` (`src/components/ui/GameScaleContainer.tsx`) on `OublietteNo9Page`, `SevenYearItchPage`, `FatesealSilverPage`, and `MastertonPage` that dynamically scales game screen dimensions preserving aspect ratio via CSS `transform: scale()`. Seamlessly centered on window resize, preventing vertical/horizontal scrolling on low-resolution viewports, expanding premium detail layouts on large 4K screens, and surrounding margins with matching Speakeasy dark radial/wood backgrounds. Supports optional `transformOrigin` and `alignItems` props to allow top-aligned flush scaling.
- **Masterson 1881:** Reverse roulette croupier rigging sim under **`src/minigames/masterson-1881/`**; configuration in `src/config/minigames/mastersonRules.ts` with Double Zero roulette geometry; isolated engine in `src/minigames/masterson-1881/engine/useMastertonEngine.ts` executing a strict 7-phase loop; 13 AI betting strategies (now including James Bond, Fibonacci, Tier-et-Tout, The Pivot, Angel's Split, and Keystone Lock), covert whiskey-glass suspicion indicators, and interactive SommelierLiveGuide Pazillus-narrated live spotlight tutorial system. React stale closure bug resolved via an engine-phase sync `useEffect` in `App.tsx` ensuring `resolveSpin` executes cleanly. Grid fully redesigned using 3 standard roulette rows, offset by spacers for green zero numbers, spanning 13 columns (12 for numbers, 1 for column bets) with direct cell-click rigging hooks. Outside bet zones (Dozens, Categories, Columns) aspect ratios corrected so they stretch organically to prevent text cutoffs. Real-time wagering loop triggers a dynamic deferred bet placement schedule (placing player bets uniquely and slower before a configurable 6-second lock countdown cutoff), slides the SVG wheel into view from a hidden slot, and displays "No More Bets!" in red. Bettor wagers render as color-coded stacked circular chips with translateY offset stacks, and seat cards display average bet size metrics. Landing outcome visual felt highlights are delayed until the ball settles, and any number cells matching the currently active rig target glow in neon yellow. Features a floating round profit/loss overlay above the Table House Ledger stack, unified to use the exact same premium glassmorphic blurred overlay styling as the player recap cards. Clicking active/evicted seat placards opens an interactive Player Dossier details modal showing their strategy description configurations and comprehensive real-time statistics (bankroll, net change, wagers, average bet size, suspicion glass, consecutive losses, herd mentality) with automatic interlocking that disables clicks and closes the modal when the wheel is spinning. When a new player joins the table, their stats and betting refs are reset to zero. End of shift conditions are fully enforced (all 30 spins completed or empty table isolation triggers game over) displaying a full-screen walnut/brass-accented statistics summary and cash-out settlement handoff.
- **Audio:** **`src/audio/clubAudioStore.ts`** (persisted) is the single settings source for **music and SFX** (Settings). The shell band starts on the intro/menu/bar/minigame routes at **30% of the user music volume until the club is entered**, then fades to the configured volume. Oubliette plays **table/UI SFX only** via `useThemeAudio`; it does not duplicate audio controls or start its own background music. **House band** continues on `/minigames/*` via `useShellBandMusic`; its active band's music queue is shuffled from runtime randomness on each load with repeat protection to play through the entire library before repeating a track. Tracks from `content/bands.json` resolve under `public/audio/bands/*` via **`bandPublicUrl`**.
- **Pazillus Interactive Live-Render Tutorial System:** A zero-screenshot, data-driven tutorial manager (`SommelierLiveGuide.tsx`) featuring a gothic/wine-centric club sommelier character named **Pazillus A. Rabellum**. The system live-renders minigame screens under non-destructive mock state configurations, drawing element spotlights using responsive gold border overlays and full-screen backdrop masks. Fully integrated across all four minigames (Oubliette No. 9, 7 Year Itch, Fateseal Silver, Masterton 1881) with dialogues and visual steps externalized in `sommelierTutorialCatalog.ts`.
...
- **Game Tips Modal:** Integrates a game-specific strategy tips overlay modal (`GameTipsModal.tsx`) launchable directly from the game's details card on the Main Menu. Curated tips configuration catalogs (`gameTipsCatalog.ts`) cover critical strategic guidelines for all four minigames. Features standard slide progress headers, ArrowLeft/ArrowRight keyboard event page-turning controls, polymorphic chiseled `ClubButton` transitions, and fully responsive layouts matching the speakeasy walnut/brass aesthetic.
- **Tests / CI:** **Vitest** + **ESLint**; Oubliette tests live under `src/minigames/oubliette-no9/**`; 7 Year Itch engine tests under `src/minigames/seven-year-itch/engine/__tests__/`; Fateseal engine tests under `src/minigames/fateseal-silver/engine/__tests__/`; Masterton engine tests under `src/minigames/masterson-1881/engine/__tests__/`; GameTipsModal component tests under `src/components/ui/__tests__/GameTipsModal.test.tsx`; shell/table helpers under `src/components/club/` and `src/game/`.
