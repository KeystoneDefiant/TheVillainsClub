...
export function createDeck(suitsList: Suit[] = SUITS, ranksList: Rank[] = RANKS): Card[] {
  const deck: Card[] = [];
...
export function createFullDeck(
  ...
  removedCards: Card[] = [],
  wildCards: Card[] = [],
  suitsList: Suit[] = SUITS,
  ranksList: Rank[] = RANKS
): Card[] {
  const deck = createDeck(suitsList, ranksList);
