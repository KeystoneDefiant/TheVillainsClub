...
- **Masterton 1881:** Reverse roulette croupier rigging sim under **`src/minigames/masterson-1881/`**; configuration in `src/config/minigames/mastersonRules.ts` with Double Zero roulette geometry; isolated engine in `src/minigames/masterson-1881/engine/useMastertonEngine.ts` executing a strict 7-phase loop; 7 AI betting strategies, covert whiskey-glass suspicion indicators, and interactive SommelierLiveGuide Pazillus-narrated live spotlight tutorial system. Stale React closure bug resolved via an engine-phase sync `useEffect` in `App.tsx` ensuring `resolveSpin` executes cleanly. Grid fully redesigned using 3 standard roulette rows, offset by spacers for green zero numbers, spanning 13 columns (12 for numbers, 1 for column bets) with direct cell-click rigging hooks. Outside bet zones (Dozens, Categories, Columns) aspect ratios corrected so they stretch organically to prevent text cutoffs. Real-time wagering loop triggers initial wagers, slides the SVG wheel into view from a hidden slot, and handles dynamic last-minute bets. Bettor wagers render as color-coded stacked circular chips with translateY offset stacks. A symmetrical 10-second circular countdown timer resets back to 10 seconds whenever a last-minute bet triggers, and auto-resolves when it expires, decelerating the swirling ball to land perfectly bottom-center on the winning slot, followed by cascading glassmorphic win/loss recap overlays nested directly inside the seat placards, and custom placard exit animations showing the leaving reasons. Symmetrical progress countdown ring shrinking symmetrically from left/right sides to bottom-center. High-fidelity white-glow winning cell highlights and interactive light-blue felt hover selection highlights on affected numbers. Fully expanded Column betting areas and perfectly equal number square grids with aligned spacer layout proportions. Hidden croupier shift logs for cleaner walnut dashboard framing. An empty-table last-chance mechanic triggers a 25% chance to spawn a wealthy gambler on any spin when the active bettors count falls to zero, keeping the shift alive. Left playfield column and right sidebar height aligned perfectly inside a premium walnut dashboard frame.
