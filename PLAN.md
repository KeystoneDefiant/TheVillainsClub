...
- **Audio:** **`src/audio/clubAudioStore.ts`** (persisted) is the single settings source for **music and SFX** (Settings). The shell band starts on the intro/menu/bar/minigame routes at **30% of the user music volume until the club is entered**, then fades to the configured volume. Oubliette plays **table/UI SFX only** via `useThemeAudio`; it does not duplicate audio controls or start its own background music. **House band** continues on `/minigames/*` via `useShellBandMusic`. Tracks from `content/bands.json` resolve under `public/audio/bands/*` via **`bandPublicUrl`**.
