...
> **Source:** `.cursor/plans/todo_511_backlog_d6d389d1.plan.md` (YAML frontmatter stripped). **Delivered in repo:** sections 1–4 (forfeit, intro, bar logo, settlement extremes). **Fateseal section 5 partial:** scatter Crossroads gate + carry, linking + sympathetic engine rules, single-omen altar, bank chip spins, Crossroads in-flow phase (full-width shop, `fatesealCrossroadsNewShop` + legacy once per visit), omen mark payout multiplier in cascade; **not yet:** bonus-spin-append-to-current-spin, grid growth in engine/UI, purchased wild/dead spin lifetimes vs session counters only, sympathetic banner UX, side payout panel, larger ritual timing.

> **Monte Carlo (2026-05-11, seed face1234, 30k spins):** `FATESEAL_SIM_BASE_ONLY=1` gives **60.75%** payout divided by paid base bet; **full** model **73.85%** — below staged doc targets; re-tune with product. If `npm run sim:fateseal` fails because `tsx` is not on PATH, use `npx tsx scripts/sim-fateseal.ts`.
...
You chose **roughly top-to-bottom** in [TODO.md](TODO.md). The list below follows that sequence. Where bullets naturally share code paths (e.g. `BarRouteState` + all cash-out sites), they are grouped into one deliverable to avoid thrash.

**Deliverable file:** After you approve, write this plan verbatim (or export from Cursor’s plan UI) to **[`511plan.md`](511plan.md)** at repo root, per your request.

---

## 1. Paused session: rejoin vs abandon (TODO §Updates)

**Current behavior:** Active table sessions live in [`src/game/clubWalletStore.ts`](src/game/clubWalletStore.ts). [`ClubTableGamesSection`](src/components/club/ClubTableGamesSection.tsx) already surfaces **Resume** per game when `activeSession` matches. [Fateseal / 7YI shell bindings](src/game/sessionSettlement.ts) expose `onPauseToClub` for stepping back to the club without settling.
...
- Ensure each minigame page’s “no session → redirect” effects remain consistent with [`isReturningToClubRef`](AGENTS.md) pattern so abandoning from the bar does not cause flicker or wrong redirects.
...
**Current:** [`IntroPage.tsx`](src/pages/IntroPage.tsx) drives phases `enter → hold → prompt → exit`, uses [`VcLogoIntroMark`](src/components/intro/VcLogoIntroMark.tsx), navigates to `/bar` after exit timing. [`MenuHazeBackground`](src/components/layout/MenuHazeBackground.tsx) is already present.
...
- **Logo component:** Extend `VcLogoIntroMark` (and related path constants in `vcLogoIntroPaths`) so draw order and fills are **sequenced** (red path animation first, grey letters after red+glow chapter). Add CSS variables or Framer sequences for **neon glow** (filter/glow + opacity keyframes), honoring **`usePrefersReducedMotion`** ([`usePrefersReducedMotion`](src/motion/usePrefersReducedMotion.ts)): reduced path = short fade, skip flicker.
- **Timing:** Reuse or extend [`motionPresetStore`](src/motion/motionPresetStore.ts) tunables so motion stays adjustable from the playground where applicable.
...
**Target screen:** Intro navigates to **`/bar`** ([`IntroPage`](src/pages/IntroPage.tsx)); confirm the actual “bar menu” layout lives in the **club menu on the bar route** (e.g. [`MainMenuPage`](src/pages/MainMenuPage.tsx) “bar-menu” branch or dedicated bar page — verify during implementation; [`BarStubPage`](src/pages/BarStubPage.tsx) may be a stub).

**Approach:** Add a compact **VC mark** header strip consistent with [`clubTokens`](src/theme/clubTokens.ts) / Mantine spacing; no duplicate heavy animation — static or subtle mark.

---

## 4. Settlement quips: extreme win / extreme loss thresholds (TODO §Updates)

**Current:** [`barSettlementTone`](src/game/barSettlementQuips.ts) uses simple multiples of buy-in and tiers (`extreme_win`: `fractionBack >= 3 || tiers >= 2`; `extreme_loss`: `fractionBack < 0.22` or zero).

**New rules:**

- **Extreme win:** only when return is within **5% of the game’s max win** for that session (use the same cap the table was opened with: `maxReturnMultipleOfBuyIn * capModifierProduct * buyIn` from [`OublietteSettlementProfile`](src/game/sessionSettlement.ts), floored/rounded consistently with settlement math).
...
- Extend [`BarRouteState`](src/game/barRouteState.ts) `lastTable` with optional **`maxReturnCredits`** (or `maxWinCredits`) computed at **settle** time from the session’s settlement snapshot (already available on each minigame page when calling `navigate("/bar", { state })`).
- Update [`buildBarRouteStateFromReturn`](src/game/barRouteState.ts) and **every** `navigate`/`buildBarRouteStateFromReturn` call site (Oubliette, 7YI, Fateseal pages) to pass the cap.
...
This block is **large**: it touches [`src/minigames/fateseal-silver/engine/cascadeEngine.ts`](src/minigames/fateseal-silver/engine/cascadeEngine.ts), [`App.tsx`](src/minigames/fateseal-silver/App.tsx), [`fatesealRules.ts`](src/config/minigames/fatesealRules.ts), specs [`Fateseal_Specs.md`](Fateseal_Specs.md), Monte Carlo [`scripts/sim-fateseal.ts`](scripts/sim-fateseal.ts), and existing engine tests under [`engine/__tests__/`](src/minigames/fateseal-silver/engine/__tests__/).

Recommended **sub-order** within Fateseal (still matches your TODO list intent, minimizes rework):

1. **Config surface first** in `fatesealRules.ts` / `resolveFatesealGameMode`: linking thresholds, bonus frequency, crossroads bonus symbol threshold (15), purchased reel lifetimes (wild 3, dead 5, mark 3), “bonus spin does not decrement purchased spin counters”, Sympathetic Vibrations payout (75× bet), grid expansion rules, dead-reel stacking rules, etc. Extend `fatesealGameConfig.gameModes` as needed so tests import constants ([`AGENTS.md`](AGENTS.md) test rule).

2. **Engine: Free Ritual / bonus spin rework** — Today `freeRitualSpinsLeft` decrements per spin and `crossroadsGate: spinCount % 3 === 0` ([`cascadeEngine.ts`](src/minigames/fateseal-silver/engine/cascadeEngine.ts)). Redesign state machine so:
...
3. **Symbol linking / cluster rules** — Non-selected symbols require **5** linked for removal; selected symbols **do not need links**; **+2× per linked matched symbol** (config-driven multiplier for future game types). Update cascade clear logic and payout aggregation; extend Vitest in [`cascadeEngine.test.ts`](src/minigames/fateseal-silver/engine/__tests__/cascadeEngine.test.ts).
...
**Practical takeaway:** With **85% already used**, treat the **Fateseal** portion of this plan as **next-cycle or extra-budget work** unless you confirm overrun or add pool. Copy this subsection into [`511plan.md`](511plan.md) when you export the plan.
...
- **Crossroads “15 bonus symbols”** — **Resolved in engine:** count is **scatter symbols on the final settled grid** per spin, accumulated in `crossroadsBonusAccum` until `fatesealProgressionRules.crossroads.scatterSymbolsToTriggerShop` (default 15); overflow carries.
