...
- **Lignée Royale:** Private 7-row playing card slot machine minigame under **`src/minigames/lignee-royale/`**; configuration in `src/config/minigames/ligneeRoyaleConfig.ts` (normal and high stakes profiles); features dynamic payline overlay SVG vectors, custom Wild Multiplier gradients (up to 5x stacking), Dead Space card exclusions, vertical motion blurs, snappy whole-reel bounce settles, dimmed non-scoring backdrop rows, swapped spin layouts, nested paylines results, and settings modal gear integration.
